...
test('dispatcher declines a cost the driver submitted from the portal', async ({ page, context, browser }) => {
  const dashboardPage = new DashboardPage(page);
  const createLoadPage = new CreateLoadPage(page);
  const customerName = `QA Decline ${Date.now()}`;
  ...
  await test.step('Dispatcher creates a load', async () => {
    ...
    await createLoadPage.fillDetails({
      customerName,
      customerEmail: 'dispatch@qa-decline.example.com',
      rate: 2100,
    });
  ...
  try {
    await test.step('Driver checks in and adds a Lumper cost at the first stop', async () => {
      ...
      await driverPortalPage.addCostNoteInput.fill('Lumper fee, no receipt available.');
  ...
  await test.step('Dispatcher declines the cost', async () => {
    const loadDetailPanelPage = new LoadDetailPanelPage(page);
    await expect(loadDetailPanelPage.panel).toContainText('Lumper');
    await loadDetailPanelPage.declineCost();
    await expect(loadDetailPanelPage.panel.getByText('Declined', { exact: true })).toBeVisible();
    await expect(loadDetailPanelPage.approveButton).toHaveCount(0);
    await expect(loadDetailPanelPage.declineButton).toHaveCount(0);
  });
});
